...
class GeometricBrownianMotion:
    ...
    def generate_geometric_brownian_motion(self):
        standard_brownian_motion = StandardBrownianMotion(
            self.num_trayectories, self.num_steps, max_time=1
        )
        ...
        for i in range(self.num_trayectories):
            for j in range(1, self.num_steps):
                geometric_brownian_motions[i, j] = np.exp(
                    (self.alpha - (1 / 2) * (self.lamda**2)) * self.t[j]
                    + self.lamda * brownian_motions[i, j]
                )
